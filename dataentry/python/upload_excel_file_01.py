...
# Load the xlsx file
excel_data_one = pd.read_excel(source_file,na_values=[''],dtype={'reg_no': str,'exam_code':str})
excel_data_one['reg_no'] = excel_data_one['reg_no'].fillna('')
excel_data_one['exam_code'] = excel_data_one['exam_code'].fillna('')
...
if solution2.values.size != 0:
    logging.info("Excel file column and table column are not matched")
...
#reset all values before process stars
...
for i in data.index:
    ...
    curr_timestamp = str(now.strftime('%d-%m-%Y %H:%M:%S'))
    try:
        #conve float into int
        reg_no          = str(data['reg_no'][i])
        #if record_count %10000 == 0:
            #print("Current Processing Records: %s",str(record_count))
        ...
        if data['reg_no'][i] == 'NA'  or len(str(reg_no)) != 11:
                continue
        #if not validate_dob(dob.strip(),i + 2):
            #error_records += 1
            #continue
        #if sys.argv[4] != exam_code_excel:
            #logging.error("Row %s: Exam Code Mismatch (Expected: %s, Actual: %s)", i + 2, sys.argv[4], exam_code_excel)
            #error_records += 1
            #continue
        else:
           ...
           # print("data_primary_key: ", data_primary_key)
            ...
            #sql = f"INSERT INTO {table_name} VALUES ({trimmed_query_placeholders})"
           # print(sql)
