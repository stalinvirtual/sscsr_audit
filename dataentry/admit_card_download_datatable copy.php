							...
							<form class="form-horizontal" action="#" method="post" id="creating_master_tables">
								...
								<div class="form-group download_options">
									...
									<div class="col-sm-6">
										...
										<!-- <input type="radio" name="download_options" value="Male" />Download Candidate
										<select name="download_options" id="download_options" required="true" class="form-control">
											<option value=""></option>
											<option value="1">download</option>
											<option value="0">not download</option>
										</select> -->
									</div>
									<div id="table_exits" class=" col-sm-4">
									</div>
...
<script>
	...
	// 				}else if(res.data.message =="Download button"){
					
	// 					$('#messageTemplate').html(res.data.html);
			...
			$.post('admit_card_download_dt_ajax.php', {
				...
				selectedtier: selectedtier,
					...
					swal.fire({
						...
						//$('#admit_submit_button').attr(res.data.html);
						//debugger;
						// $('#admit_submit_button').attr('id', 'download_btn');
						// $('#admit_submit_button').html(res.data.html);
						//$('#messageTemplate').html(res.data.html);
					...
					// })
					// var blob = new Blob([res.data.html], { type: 'text/csv' });
                    // var link = document.createElement('a');
                    // link.href = window.URL.createObjectURL(blob);
                    // link.download = res.data.html;
                    // link.click();

					var uniqueIdentifier = res.data.html;
            var downloadLink = document.createElement("a");
            downloadLink.href = "csv/" + uniqueIdentifier + ".csv";
			//if(res.data.download_option == 1){
				 downloadLink.download = uniqueIdentifier + ".csv";
				//downloadLink.download =  res.data.tableName + '_'  + res.data.selectedtier + '_downloaded.csv';
			// }else{
			// 	downloadLink.download =  res.data.tableName + '_'  + res.data.selectedtier + '_not_downloaded.csv';
			// }
