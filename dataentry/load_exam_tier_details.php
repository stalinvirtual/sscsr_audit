...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) {
	...
	join tier_master tm on cast(dtm.tier_id as char(255)) =  cast(tm.tier_id as char(255))  order by dbm.table_id desc ";
	$result = getAll($query);
		...
		if ($row->table_type == 'tier') {
			if ($row->tier_id == '1') {
				$sql = "SELECT distinct LEAST(
					(SELECT MIN(date) FROM (VALUES (date1::date-INTEGER '$row->no_of_days' )) AS dates(date))
...
and date2 !='NA'  and date3 !='NA' and date4 !='NA'  order by yesterday_date asc limit 1";
			...
			$sql = "SELECT distinct LEAST(
				(SELECT MIN(date) FROM (VALUES (skill_test_date::date-INTEGER '$row->no_of_days' )) AS dates(date))
			  ) AS yesterday_date from $row->table_name where tier_id = '$row->tier_id' order by yesterday_date asc limit 1";
			$result = getAll($sql);
			$sql2 = "SELECT distinct LEAST(
			(SELECT MIN(date) FROM (VALUES (skill_test_date::date )) AS dates(date))
		  ) AS exam_date from $row->table_name where tier_id = '$row->tier_id' order by exam_date asc limit 1";
			$result2 = getAll($sql2);
		} else if ($row->table_type == 'dme') {
			$sql = "SELECT distinct LEAST(
				(SELECT MIN(date) FROM (VALUES (date_of_dme::date-INTEGER '$row->no_of_days' )) AS dates(date))
			  ) AS yesterday_date from $row->table_name where tier_id = '$row->tier_id' order by yesterday_date asc limit 1";
			$result = getAll($sql);
			$sql2 = "SELECT distinct LEAST(
			(SELECT MIN(date) FROM (VALUES (date_of_dme::date )) AS dates(date))
		  ) AS exam_date from $row->table_name where tier_id = '$row->tier_id' order by exam_date asc limit 1";
			$result2 = getAll($sql2);
		} else if ($row->table_type == 'pet') {
			$sql = "SELECT distinct LEAST(
				(SELECT MIN(date) FROM (VALUES (pet_date::date-INTEGER '$row->no_of_days' )) AS dates(date))
			  ) AS yesterday_date from $row->table_name where tier_id = '$row->tier_id' order by yesterday_date asc limit 1";
			$result = getAll($sql);
			$sql2 = "SELECT distinct LEAST(
			(SELECT MIN(date) FROM (VALUES (pet_date::date )) AS dates(date))
		  ) AS exam_date from $row->table_name where tier_id = '$row->tier_id' order by exam_date asc limit 1";
			$result2 = getAll($sql2);
		} else if ($row->table_type == 'dv') {
			$sql = "SELECT distinct LEAST(
				(SELECT MIN(date) FROM (VALUES (dv_date::date-INTEGER '$row->no_of_days' )) AS dates(date))
			  ) AS yesterday_date from $row->table_name where tier_id = '$row->tier_id' order by yesterday_date asc limit 1";
			$result = getAll($sql);
			$sql2 = "SELECT distinct LEAST(
			(SELECT MIN(date) FROM (VALUES (dv_date::date )) AS dates(date))
		  ) AS exam_date from $row->table_name where tier_id = '$row->tier_id' order by exam_date asc limit 1";
			$result2 = getAll($sql2);
		}
		$admitcard_enable_date = @$result[0]->yesterday_date;
		$current_exam_date = @$result2[0]->exam_date;
	//$current_exam_date = '2023-06-23';
		...
		if ($row->updated_on == "") {
			$time = '<span style="color:black;font-size:13px">' . date("d-m-Y H:i:s") . '</span>';
		} else {
			$time = '<span style="color:black;font-size:13px">' . date("d-m-Y H:i:s", strtotime($row->updated_on)) . '</span>';
		}
		$output .= '<td>
...
<script type="text/javascript" language="javascript">
	$(document).ready(function () {
		...
		$('#exam_data').on('click', '.publish-button', function (event) {
			...
			var dateComponents = exam_date.split('-');
			// Create a new Date object using the components in MM-DD-YYYY format
			var dateObj = new Date(`${dateComponents[1]}-${dateComponents[0]}-${dateComponents[2]}`);
			// Get the year, month, and day from the date object
			var year = dateObj.getFullYear();
			var month = String(dateObj.getMonth() + 1).padStart(2, '0'); // Months are zero-based, so add 1
			...
			// }  
			swal.fire({
				title: '<strong> Do you want to change Count of Days?</strong>',
				showCloseButton: true,
				...
				if (result.value) {
					...
					Swal.fire({
						...
						didOpen: () => {
							// Find the input element within the dialog
							const input = document.querySelector('.swal2-input');
							// Add the maxlength attribute to the input element
							input.setAttribute('maxlength', '2');
							// Set the input element's type to "number" to show numeric keypad on mobile devices
							input.type = 'number';
						},
						preConfirm: (login) => {
							const isValidInput = /^\d{1,2}$/.test(login);
							if (!isValidInput) {
								Swal.showValidationMessage('Please enter a valid number with 1 to 2 digits.');
								return false;
							} 
							debugger;
							var today = new Date();
							...
							var d2 = new Date(ymdFormat);
							var diff = d2.getTime() - d1.getTime();
							// if (diff < 0) {
							// 	alert("Difference is negative");
							// 	return false;
							// }
							// else{
		...
		$('#exam_data').on('click', '.stop_status_class', function (event) {
			...
			if (title == "Stop") {
				var new_title = "Unpublish"
			}
			else {
				var new_title = "Publish"
			}
			swal.fire({
				title: '<strong> Do you want to ' + new_title + ' the Data ?</strong>',
				html: exam_name,
				showCloseButton: true,
				confirmButtonText: 'Yes',
				cancelButtonText: 'No',
